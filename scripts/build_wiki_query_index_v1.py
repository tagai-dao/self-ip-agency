...
from message_hygiene_v1 import sanitize_summary
...
def _build_recent_events(limit: int = 10) -> list[dict[str, Any]]:
    ...
    for line in reversed(lines):
        ...
        try:
            e = json.loads(line)
            safe_summary, _summary_flag, _summary_meta = sanitize_summary(
...
def main() -> int:
    ...
    # Emit event
    event_summary, _summary_flag, _summary_meta = sanitize_summary(
        f"artifacts={len(index['artifacts'])} health={index['health']['overall']}",
        files_changed=[str(INDEX_PATH.relative_to(WORKSPACE))],
        tests_passed=True,
        blockers=[],
    )
    append_wiki_event(
